...
client.on('messageCreate', async (message) => {
  ...
  if (!roleName) {
    return message.reply('❌ Zadej název role. Použití: `^addrole <role> @user1 @user2 ...`');
  }
  ...
  // Kontrola, zda má bot vyšší roli než přidávaná role
  if (role.position >= message.guild.members.me.roles.highest.position) {
    return message.reply(`❌ Nemohu přidat roli **${role.name}** — je výše než moje nejvyšší role.`);
  }
  ...
  if (!mentionedMembers || mentionedMembers.size === 0) {
    return message.reply('❌ Neotagoval jsi žádné uživatele. Použití: `^addrole <role> @user1 @user2 ...`');
  }

  // Přidej roli každému zmíněnému uživateli
  ...
  if (results.success.length > 0) {
    reply += `✅ Role **${role.name}** přidána (${results.success.length}): ${results.success.join(', ')}\n`;
  }
